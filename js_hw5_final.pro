...
;expected number of photons in each bin
; then you loop through starting with the first bin
; if it is 10 expected photons
; call randomn and multiply by root N
;n+rootN *randomn(seed)

; Titles for plot. EDIT them here only
title = "# of photons gathered from a Sun-like star and two random stars"
...
temp_array = [T, T+T*randomu(seed), T*randomu(seed)]
...
for ii = 0,2 do begin
  ...
  xyouts, max(lambda, indexofmax)+1000, max(nphoton), 'Star'+string(ii+1) ; not sure how to do this
  
  for jj=0, nbins-1 do begin
    noise = nphoton
    dnoise = sqrt(noise[jj])*randomn(seed)
    noise[jj]=noise[jj]+dnoise
    oplot, lambda/nanometer, noise, color=color, psym=10
    endfor
